...
**Tarefa:**
Analise o conteúdo do edital fornecido a seguir и extraia as informações conforme o JSON Schema. Se alguma informação não for encontrada, omita o campo ou preencha com um valor nulo ou vazio.
...
export const verticalizeEdital = async (userInput: string | FilePart, cargo: string, signal: AbortSignal): Promise<string> => {
    ...
    if (!API_KEY) {
        throw new Error("A chave de API (API_KEY) não está configurada no ambiente. Por favor, adicione-a como uma variável de ambiente nas configurações do seu projeto Vercel.");
    }
    const ai = new GoogleGenAI({ apiKey: API_KEY });
    
    try {
        // FIX: The `generateContent` method does not support an AbortSignal directly.
        // This implements cancellation by racing the API call against a promise that rejects when the signal is aborted.
        // This ensures the application logic stops even if the underlying network request continues.
        if (signal.aborted) {
